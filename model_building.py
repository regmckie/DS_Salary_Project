...
#
# FILE DESCRIPTION:
# This is the next step after data exploration. After EDA, we create multiple models
# and compare their results. We create multiple linear regression models (one of them with Lasso regression)
# and a random forest model. After creating the random forest model, we tune its parameters to see which set
# of parameters and values can best predict our data. Next, we test the ensembles and see which one has the
# lowest MAE (mean absolute error). Finally, we store the random model with the best estimators (parameters
# and values) to put into production later on.
...
pickl = {'model': gs.best_estimator_}  # Store model in  dict, like {'model': <your trained model>}
pickle.dump(pickl, open('model_file' + '.p', 'wb'))  # Saves model to disk so that it can be reused without retraining

file_name = 'model_file.p'
with open(file_name, 'rb') as pickled:
    # Load & extract the model
...
# Get the X_test data to put into 'data_input.py' file
#float_arr = X_test.iloc[1,:].to_numpy().astype(float)
